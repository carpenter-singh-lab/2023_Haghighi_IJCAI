...
def display_instances(
    ...
    if not N:
        ...
        print(" *** " + str(N) + " instances to display *** \n")
    ...
    for i in range(N):
        ...
        if not captions:
            ...
            caption = captions[i]
        ax[0].text(
            x1, y1 + 8, caption, color=label_color, size=8, backgroundcolor="none"
        )
...
def CP_to_RGB_single(im_cp):
    ...
    for i in range(im_cp.shape[2]):
        ...
        image_color = (
            skimage.color.gray2rgb(image_gray).astype(float) / 255
        ) * channel_colors[i]
...
def colorize_image(img, col):
    ...
    # colorize
    #     print("h1 ",img_float.max(),img_float.min())
    img_col_float = np.reshape(img_float, img_float.shape + (1,)) * col
    #     print("h2 ",img_col_float.max(),img_col_float.min())
    ...
    #     print("h3 ",img_col_byte.max(),img_col_byte.min())
    return img_col_byte
...
def normalize(img):
    ...
    image = skimage.exposure.rescale_intensity(
        img, in_range=(low, high), out_range=(0, 255)
    ).astype(np.uint8)
